...
# images path
const IMAGES_PATH: String = "res://asset/image"
const ATLAS_PATH: String = "asset/map"

func _run() -> void:
	var spritesheet_names: PoolStringArray = get_spritesheet_names()
	
	print(spritesheet_names)
...
func get_path(file_name_included: bool = true, folder: String = "") -> String:
	...
	var path: String = "res://"
	path = path.plus_file(ATLAS_PATH)
	for part in name:
...
func edit_texture(spritesheet: StreamTexture) -> void:
  ...
  # texture folder path (without the filename) plus a 'texture' subfolder
	var folder_path: String = get_path(false)
  # texture full path
	var resource_path: String = get_path(true) + ".tres"
